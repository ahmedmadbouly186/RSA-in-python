...
while i < 65:
    ...
    s = "{:,}".format(et-st)
    print(f"{bcolors.BOLD}{bcolors.WARNING} the time needed to breke {bcolors.FAIL}{bit_size}{bcolors.WARNING} bits key is: {s} millis")
    print(f"{bcolors.ENDC}-------------------------------------------------------")
    with open('filename.txt', 'a') as f:
        sys.stdout = f
        if((temp_p == p) and (temp_q == q) and (temp_phai == phai) and (temp_d == d)):
            print(f"Yes I could breake this ciphper eassy")
        else:
            print(f"No I could not break this ciphper")
        s = "{:,}".format(et-st)
        print(
            f"time needed to breke {bit_size} bits key is: {s} millis")
        print(f"-------------------------------------------------------------------------")
